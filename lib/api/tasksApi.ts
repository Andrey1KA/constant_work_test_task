...
export async function llmSuggestCategory(body: {
  title: string;
  description?: string | null;
}) {
  const { data } = await client.post<{ category: string; reasoning?: string }>(
...
export async function llmDecompose(body: {
  title: string;
  description?: string | null;
}) {
  const { data } = await client.post<{ subtasks: string[] }>(
    '/llm/decompose',
    body
  );
  return data;
...
export async function llmSuggestPriority(body: {
  title: string;
  description?: string | null;
  dueDate?: string | null;
}) {
