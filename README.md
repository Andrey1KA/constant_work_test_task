...
Тесты: Vitest (unit, файлы `src/**/*.test.ts`), Playwright (E2E, US-1…US-6, каталог `e2e/`).
...
Один процесс Next.js: App Router в `src/app/`, API в `src/app/api/v1/`.

Разработка: `npm run dev` → [http://localhost:3000](http://localhost:3000) (редирект на `/tasks`).

Продакшен: `npm run build` → `npm start`.

Тесты: `npm run test:run` (Vitest), `npm run test:e2e` (Playwright), либо подряд: `npm run test:all`.

---

## 4. Описание принятых архитектурных решений

- Слои: Route Handler (`src/app/api/…`) → `src/lib/services` (Zod, правила) → `src/lib/repositories` + Prisma → SQLite; LLM в `src/lib/services/llmService.ts`. UI страниц — `src/views/`, общие компоненты — `src/components/`, состояние доски — `src/state/tasksBoard/`.
